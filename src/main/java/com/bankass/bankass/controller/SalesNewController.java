...
public class SalesNewController extends BaseController {
	...
	private void watch() {
		if (!(WindowsUtils.isTextFieldEmpty(emailTextField)) && !WindowsUtils.isComboBoxSelected(clientComboBox)) {
	...
	public void onSelectClient(Client client) {
		...
		if (client != null) {
			...
			if (!client.getPhones().isEmpty()) {
				WindowsUtils.setTextInTextField(phoneTextField,
	...
	private void updateTextFields() {
		...
		if (sale.getPhone() != null) {
			WindowsUtils.setTextInTextField(phoneTextField, sale.getPhone().getNumber());
		}
	...
	public void onSave() {
		...
		try {
			saleService.save(EntityFactory.createSale(sale, WindowsUtils.getTextFromLabel(saleCode),
					...
					(String) WindowsUtils.getSelectedComboBoxItem(stateComboBox), Integer.valueOf(numUnits),
					Double.valueOf(total), phone, (Client) WindowsUtils.getSelectedComboBoxItem(clientComboBox), items,
					null), e -> {
