...
public class RootController extends BaseController {
	...
	private void onAbout() throws Exception {
		WindowsUtils.openNewWindow(AboutController.PATH_FXML, getWindowTitle(AboutController.ABOUT_TITLE_KEY),
				AboutController.PATH_ICON, null, Modality.APPLICATION_MODAL);
	}

	@FXML
	private void onSettings() throws Exception {
		WindowsUtils.openNewWindow(SettingsController.PATH_FXML, getWindowTitle(SettingsController.SETTINGS_TITLE_KEY),
				SettingsController.PATH_ICON, null, Modality.WINDOW_MODAL);
		stage.close();
	}
	...
	private void onLogout() throws Exception {
		...
		userService.setUserAsSignOut(e -> {
			try {
				WindowsUtils.openNewWindow(LoginController.PATH_FXML, getWindowTitle(LoginController.LOGIN_TITLE_KEY),
						LoginController.PATH_ICON, null, Modality.WINDOW_MODAL);
				onClose();
			} catch (Exception e1) {
				e1.printStackTrace();
			}
