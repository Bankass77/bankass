...
public class SupplierNewController extends BaseController {
	...
	private boolean isRequiredTextFieldsFilled() {
		return !WindowsUtils.isTextFieldEmpty(nameTextField) || !WindowsUtils.isTextFieldEmpty(emailTextField)
				|| !WindowsUtils.isTextFieldEmpty(cepTextField) || !WindowsUtils.isTextFieldEmpty(streetTextField)
				|| !WindowsUtils.isTextFieldEmpty(numberTextField) || !WindowsUtils.isTextFieldEmpty(districtTextField)
				|| !WindowsUtils.isTextFieldEmpty(cityTextField) || !WindowsUtils.isTextFieldEmpty(stateTextField);
	}
	...
	public void onSave() {
		...
		if (isAddressFilled()) {
			address = EntityFactory.createAddress(WindowsUtils.getTextFromTextField(streetTextField),
					...
					WindowsUtils.getTextFromTextField(cityTextField), WindowsUtils.getTextFromTextField(stateTextField),
		...
		try {
			supplierService
					.save(EntityFactory.createSupplier(supplier, WindowsUtils.getTextFromTextField(nameTextField),
							WindowsUtils.getTextFromTextField(emailTextField), address), e -> {
								WindowsUtils.createDefaultDialog(container, "Sucess", "Supplier save with sucess.",
										() -> {
											stage.close();
										});
							}, null);
