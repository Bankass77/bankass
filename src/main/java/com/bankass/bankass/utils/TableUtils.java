...
public class TableUtils {
	...
	public static <T extends RecursiveTreeObject<T>> void configurePagination(JFXTreeTableView<T> tableView,
			ObservableList<T> data, Pagination pagination) {
		pagination.setPageFactory((index) -> createPage(tableView, data, pagination, index));
	}

	private static <T extends RecursiveTreeObject<T>> Node createPage(JFXTreeTableView<T> tableView,
			ObservableList<T> data, Pagination pagination, int pageIndex) {

		int fromIndex = pageIndex * ROW_PER_PAGE;
		int toIndex = Math.min(fromIndex + ROW_PER_PAGE, data.size());
		tableView.setRoot(new RecursiveTreeItem<>(FXCollections.observableArrayList(data.subList(fromIndex, toIndex)),
	...
	public static <T extends RecursiveTreeObject<T>> void configureEditAndRemoveState(JFXTreeTableView<T> table,
			JFXButton edit, JFXButton remove) {
		table.setOnMouseClicked((e) -> {
			updateEditAndRemoveButtonState(table, edit, remove);
		});
	}

	public static <T extends RecursiveTreeObject<T>> void configureButtonState(JFXTreeTableView<T> table,
			JFXButton button) {
		table.setOnMouseClicked((e) -> {
	...
	public static <T extends RecursiveTreeObject<T>> void updateEditAndRemoveButtonState(JFXTreeTableView<T> table,
			JFXButton edit, JFXButton remove) {
	...
	private static <T extends RecursiveTreeObject<T>> void updateButtonState(JFXTreeTableView<T> table,
			JFXButton button) {
		if (table.getSelectionModel().selectedItemProperty().get() != null) {
	...
	public static <T extends RecursiveTreeObject<T>> void configureTableSearch(TextField textField,
			JFXTreeTableView<T> tableView, ITableSearchTest<T> test) {
	...
	public static <T extends RecursiveTreeObject<T>> void removeItemFromTable(IBaseService<?> service, long id,
			JFXTreeTableView<T> table, ObservableList<T> data, StackPane container) {
		try {
			service.delete(id, e -> {
	...
	public static <T extends RecursiveTreeObject<T>, D> void editItemFromTable(JFXTreeTableView<T> table, D item,
			String key, String pathFXML, String title, String icon) throws Exception {
